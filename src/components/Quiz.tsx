...
export const Quiz: React.FC<Props> = ({ selectedFormOptions }) => {
  ...
  const getQuestions = async (url: string) => {
    try {
        ...
        return setQuestionsData(questionsData.results)
    } catch(err) {
        console.log(err)
    }
  }
    ...
    <>
     ...
     <h1 className="title">1/{selectedFormOptions.numberOfQuestion}</h1>
      ...
      <ButtonContainer>
        ...
        <progress className="progress is-success is-medium" value={1} max={selectedFormOptions.numberOfQuestion}></progress>  
      </AppContainer>
    </>
  )
}
